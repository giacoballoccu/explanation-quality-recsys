...
'''
def maximize_ED_optimize_ETR(path_data, alpha):
    pred_path = path_data.pred_paths
    for uid, pid_list in pred_path.items():
        candidates = []
        best_candidates = []
        ...
        #Sort every path type bin by a mixed score based on explaination time relevance and item relevance
        for bin_type, path_list in bins.items():
            path_list.sort(key=lambda x: (x[0]*(1-alpha)) + (time_relevance_single(path_data, x[-1]) * alpha), reverse=True)

        #Pick the best path from every type bin in order to maximize diversity
        for bin_type, path_list in bins.items():
            if len(path_list) == 0: continue
            best_candidate = path_list[0]
            rec_pid = get_rec_pid(best_candidate)
        ...
        #Fill the topk with the best paths regardless of the path type
        while len(best_candidates) < 10:
            if len(candidates) == 0:
                break
        ...
        #Rearrange the topk based on the metric
        best_candidates.sort(key=lambda x: (x[0] * (1 - alpha)) + (time_relevance_single(path_data, x[-1]) * alpha),
                       reverse=True)
...
def maximize_ED_optimize_ES(path_data, alpha):
    pred_path = path_data.pred_paths
    for uid, pid_list in pred_path.items():
        candidates = []
        best_candidates = []
        ...
        for candidate in candidates:
            candidate_path_type = get_path_type(candidate[-1])
            if candidate_path_type not in bins:
                bins[candidate_path_type] = []
            bins[candidate_path_type].append(candidate)
        ...
        # Rearrange the topk based on the metric
        best_candidates.sort(key=lambda x: (x[0] * (1 - alpha)) + (explaination_serentipety_single(path_data, x[-1]) * alpha),
                             reverse=True)
...
def maximize_ED_optimize_all_exp_quality(path_data, alpha):
    pred_path = path_data.pred_paths
    for uid, pid_list in pred_path.items():
        candidates = []
        best_candidates = []
        ...
        scores_list = [candidate[0] for candidate in candidates]
        for candidate in candidates:
            candidate[0] = (candidate[0] - (min(scores_list))) / (max(scores_list) - (min(scores_list)))
        ...
        # Sort every path type bin by a mixed score based on explaination time relevance and item relevance
        for bin_type, path_list in bins.items():
            path_list.sort(key=lambda x: (x[0] * (1 - alpha)) + ((time_relevance_single(path_data, x[-1]) + explaination_serentipety_single(path_data, x[-1])) * alpha),
                           reverse=True)
        ...
        # Rearrange the topk based on the metric
        best_candidates.sort(key=lambda x: (x[0] * (1 - alpha)) + ((time_relevance_single(path_data, x[-1]) + explaination_serentipety_single(path_data, x[-1])) * alpha),
                             reverse=True)
...
def experiment(path_data, q=0.8,t=0.6,s=0.4,d=0.2):
    pred_path = path_data.pred_paths
    for uid, pid_list in pred_path.items():
        candidates = []
        best_candidates = []
        ...
        for bin_type, path_list in bins.items():
            path_list.sort(
                key=lambda x: (x[0] * q) + (((time_relevance_single(path_data, x[-1])*t) + (explaination_serentipety_single(path_data, x[-1])*s))),
                reverse=True)
        ...
        while len(best_candidates) < 10:
            ...
            for bin_type, path_list in bins.items():
                ...
                while rec_pid in best_candidates_pids:
                    ...
                    rec_pid = get_rec_pid(candidate[-1])
                if len(path_list) == 0: continue
                bonus = d if get_path_type(candidate[-1]) not in ptype_seen else 0
                score = candidate[0] + bonus
                if score > best_score:
                    best_score = score
                    best_type = get_path_type(candidate[-1])
