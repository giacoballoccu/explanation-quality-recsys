...
if __name__ == '__main__':
    ...
    soft_optimizations = ["softED", "softES", "softETR"]
    alpha_optimizations = ["EDopt", "ESopt", "ETRopt", "ED_ES_opt", "ED_ETR_opt", "ES_ETR_opt", "ED_ES_ETR_opt"]
    ...
    if args.eval_baseline == True:
        ...
        print_rec_metrics(rec_metrics_before)
        #Exp Quality
        exp_metrics_before = {}
    ...
    if chosen_optimization in soft_optimizations:
        ...
        for optimization in soft_optimizations:
            ...
            if chosen_optimization == "softED":
                soft_optimization_ED(path_data)
            elif chosen_optimization == "softES":
                soft_optimization_ES(path_data)
            elif chosen_optimization == "softETR":
                soft_optimization_ETR(path_data)
    ...
    if chosen_optimization in alpha_optimizations:
        for optimization in alpha_optimizations:
            chosen_optimization = optimization
            if args.log_enabled == True:
                log_path = log_base_path + chosen_optimization + ".txt"
                log_file = open(log_path, "w+")
            ...
            if args.save_after_rec_quality_distributions or args.save_after_exp_quality_distributions:
                ...
                distribution_file = open(filename, 'w+')
            ...
            for alpha in alphas:
                ...
                if chosen_optimization == "EDopt":
                    optimize_ED(path_data, alpha)
                elif chosen_optimization == "ESopt":
                    optimize_ES(path_data, alpha)
                elif chosen_optimization == "ETRopt":
                    optimize_ETR(path_data, alpha)
                elif chosen_optimization == "ED_ES_opt":
                    optimize_ED_ES(path_data, alpha)
                elif chosen_optimization == "ED_ETR_opt":
                    optimize_ED_ETR(path_data, alpha)
                elif chosen_optimization == "ES_ETR_opt":
                    optimize_ETR_ES(path_data, alpha)
                elif chosen_optimization == "ED_ES_ETR_opt":
